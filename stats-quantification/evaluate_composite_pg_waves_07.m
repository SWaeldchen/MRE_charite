...
%% DEMO VALS FOR LOW PASS OVERSHOOT (SOFTER FEATURES): PHASE_WIDTH 11 MAGVECTOR_WIDTH 15 PHASE_COEFF 15 MAGVECTOR_COEF 100
  ...
  spike5 = circshift(fspecial('gaussian', [N 1], phase_width*1) * coef, 1 + round(randn*10));
  spike6 = circshift(fspecial('gaussian', [N 1], phase_width*1.2) * 0.6 * coef, N/16 + round(randn*10));
  spike7 = circshift(fspecial('gaussian', [N 1], phase_width*1.3) * 0.6 *coef, N/8 + round(randn*10));
  spike8 = circshift(fspecial('gaussian', [N 1], phase_width*1.4) *coef, 3*N/16 + round(randn*10));
  spike9 = circshift(fspecial('gaussian', [N 1], phase_width*1.2) * 0.8* coef, N/4 + round(randn*10));
