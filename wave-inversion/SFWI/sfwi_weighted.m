function [mu_sfwi, mu_helm] = sfwi_weighted(U, freqvec, spacing, xyz_order, nohelm, kern_ord)
...
weights = spdiags(abs(U(:)).^4, 0, numel(U), numel(U));

%K_helm = weights*K1_helm*K2_helm;
K_sfwi = weights*K1_sfwi*K2_sfwi;
...
%u_sfwi = K_sfwi \ f;
u_sfwi = K_sfwi \ (weights*f);
%u_sfwi = lsqr(K_sfwi, f, 1e-15, 100000);
%u_sfwi = lsqr(K_sfwi, weights*f, 1e-15, 5000);
