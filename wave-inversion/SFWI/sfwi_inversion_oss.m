function [mu_sfwi, mu_helm, es] = sfwi_inversion_oss(U, freqvec, spacing, xyz_order, nohelm, kern_ord)
...
%onevec(1) = 0;
%onevec(end) = 0;
es=zeros(sz(1),sz(2),sz(3),nfreqs);
% set variable scope
f = [];
K1_helm = [];
K1_sfwi = [];
...
for n = 1:nfreqs
    ...
    es(:,:,:,n)=sqrt(2)/3*sqrt((cell2mat(E{1,1})-cell2mat(E{2,2})).*conj(cell2mat(E{1,1})-cell2mat(E{2,2}))+(cell2mat(E{1,1})-cell2mat(E{3,3})).*conj(cell2mat(E{1,1})-cell2mat(E{3,3}))+(cell2mat(E{2,2})-cell2mat(E{3,3})).*conj(cell2mat(E{2,2})-cell2mat(E{3,3}))+6*(cell2mat(E{1,2}).*conj(cell2mat(E{1,2}))+cell2mat(E{1,3}).*conj(cell2mat(E{1,3}))+cell2mat(E{2,3}).*conj(cell2mat(E{2,3}))));        
...
es2(:,:)=reshape(es,sz(1)*sz(2)*sz(3),nfreqs);
es3=[];
for n=1:nfreqs
    es3=[es3; repmat(es2(:,n)./sum(es2,2),3,1)];
end
oss_w=spdiags(es3(:),0,3*nfreqs*sz(1)*sz(2)*sz(3),3*nfreqs*sz(1)*sz(2)*sz(3));
...
%u_helm = K_helm \ f;
if (nohelm == 0)
    u_helm = lsqr(oss_w*K_helm, oss_w*f, 1e-15, 100000);
...
u_sfwi = lsqr(oss_w*K_sfwi, oss_w*f, 1e-15, 20000);
