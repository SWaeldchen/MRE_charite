function [mu_sfwi, mu_helm] = sfwi_super_tik(U, freqvec, spacing, xyz_order, nohelm, kern_ord, niter)
...
K2_sfwi = assemble_sfwi_super_matrix(sz(1:3));
disp('done')
K2_helm = K2_sfwi(1:N, :);
...
%u_helm = K_helm \ f;
if (nohelm == 0)
    u_helm = lsqr(K_helm, f, 1e-6, 100000);
...
tic
% TIK
pad = size(K_sfwi, 1) - numel(f);
f = [f; zeros(pad,1)];
%u_sfwi = lsqr(K_sfwi, f, 1e-15, 100000);
% EARLY EXIT FOR TESTING
[initial_guess_m, initial_guess_p] = helmholtz_inversion(interp_3d(U,2), freqvec, spacing/2, 3, 4, 1);
initial_guess = initial_guess_m.*cos(initial_guess_p) + 1i*initial_guess_m.*sin(initial_guess_p);
u_sfwi = lsqr(K_sfwi, f, 1e-12, niter, [], [], vec(initial_guess));
toc
