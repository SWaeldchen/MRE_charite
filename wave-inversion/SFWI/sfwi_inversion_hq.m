function mu_sfwi = sfwi_inversion_hq(U, freqvec, spacing, xyz_order, a, b, tol)
...
[mag, phi] = finish_from_hodge(U, freqvec, spacing, 3, 0, 0);
%x0 = ones([sz(1) sz(2) sz(3)]);
x0_cplx = mag.*cos(phi) + 1i.*mag.*sin(phi);
x0 = reshape(x0_cplx, [sz(1) sz(2) sz(3)]);
phi = 'sqr';
MxIt = 10;
u_sfwi = HQ_Multiplicative(x0,f,K_sfwi,phi,a,b,tol,MxIt);
