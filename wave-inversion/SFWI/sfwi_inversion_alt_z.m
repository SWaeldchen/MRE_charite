function [mu_sfwi, mu_helm] = sfwi_inversion_alt_z(U, freqvec, spacing, xyz_order, nohelm, kern_ord)
...
for n = 1:nfreqs
    ...
    % INFINITESIMAL STRAIN TENSOR - ALT Z FOR FEM
    E = {   { x_x         }  {(x_y + y_x)/2}  {(x_z + x_z)/2}    ;   ...
            {(y_x + x_y)/2}  { y_y         }  {(y_z + y_z)/2}    ;   ...
            {(z_x + z_x)/2}  {(z_y + z_y)/2}  {z_z          }     };
...
function [mu_sfwi, mu_helm] = sfwi_inversion_3(U, freqvec, spacing, xyz_order)
