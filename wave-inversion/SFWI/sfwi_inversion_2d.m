function [mu_sfwi, mu_helm] = sfwi_inversion_2d(U, freqvec, spacing, xyz_order, nohelm, kern_ord)
...
for n = 1:nfreqs
    ...
    E = {   { x_x         }  {(x_y + y_x)/2}  {zeros(size(x_z))}    ;   ...
            {(y_x + x_y)/2}  { y_y         }  {zeros(size(y_z))}    ;   ...
            {zeros(size(z_x))}  {zeros(size(z_y))} {zeros(size(z_z))} };
