function [mu_sfwi, mu_helm] = sfwi_super_4(U, freqvec, spacing, xyz_order, kern_ord)
...
K2_helm_sup = sparse(size(K2_helm,1), size(K2_helm,2)*2);
K2_helm_sup(:,1:2:end) = K2_helm;
K2_helm_sup(:,2:2:end) = K2_helm*0.5;
K2_helm_sup(2:end,2:2:end) = K2_helm(1:end-1)*0.5;
%}
...
K2_sfwi_sup = sparse(size(K2_sfwi,1), size(K2_sfwi,2)*2);
K2_sfwi_sup(:,1:2:end) = K2_sfwi;
K2_sfwi_sup(:,2:2:end) = K2_sfwi*0.5;
K2_sfwi_sup(2:end,2:2:end) = K2_sfwi(1:end-1,:)*0.5;
K_sfwi_sup = K1_sfwi*K2_sfwi_sup;
...
%}
%u_sfwi = K_sfwi \ f;
u_sfwi = lsqr(K_sfwi_sup, f, 1e-15, 10000);
