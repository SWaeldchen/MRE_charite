function [mu_sfwi, mu_helm] = sfwi_inversion_real(U, freqvec, spacing, xyz_order)
...
for n = 1:nfreqs
    ...
    U_x = real(double(U(:,:,:,xyz_order(1),n))); %nifti complex format is single
    U_y = real(double(U(:,:,:,xyz_order(2),n)));
    U_z = real(double(U(:,:,:,xyz_order(3),n)));
