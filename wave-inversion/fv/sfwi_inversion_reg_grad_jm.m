function [mu_sfwi, mu_helm] = sfwi_inversion_reg_grad_jm(U, freqvec, spacing, xyz_order, nohelm, kern_ord)
...
%     [z_derivs, U_z] = gradestim4d(double(U(:,:,:,xyz_order(3),n)), spacing);
    x_derivs = wgrad3d(double(squeeze(U(:,:,:,xyz_order(1),n))), spacing);
    y_derivs = wgrad3d(double(squeeze(U(:,:,:,xyz_order(2),n))), spacing);
    z_derivs = wgrad3d(double(squeeze(U(:,:,:,xyz_order(3),n))), spacing);
    
    U_x = U(2:end-1,2:end-1,2:end-1,xyz_order(1),n);
    U_y = U(2:end-1,2:end-1,2:end-1,xyz_order(2),n);
    U_z = U(2:end-1,2:end-1,2:end-1,xyz_order(3),n);
