function [magm_orig, phase_orig_vol, magm_moco, phase_moco_vol] = motion_correction_pipeline(acq_inf)
% environment

PROJ_DIR = fullfile('/media/ericbarnhill/datentausch/eric');

DATA_SUB = fullfile(PROJ_DIR,acq_inf.folder_name);
...
mag_series = acq_inf.mag_series;
phase_series = acq_inf.phase_series;
mprage_series = acq_inf.mprage_series;
...
disp('mdev MRE data...');
    ...
    if ~exist(fullfile(MOCO_SUB,'rmag_orig_4D.nii'),'file')
        convert_ri2pm(RAWN_SUB,MOCO_SUB,'rRL');
        convert_ri2pm(RAWN_SUB,ORIG_SUB,'RL');
    end
