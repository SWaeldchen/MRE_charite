...
function [c, Lambda, amplitude, shearWaveField, waveField] = kMDEV_BM3D_wavelet( MRsignal, inplaneResolution, frequency)

% smooth the complex MREsignal---------------------------------------------
sigma = 2.75 * 10^-3;%[m] filter strength for gaussien smoothing
MRsignalSmoothed = smoothMRsignal_BM3D_nosuppress( MRsignal );
...
%temporalWaveField = imag(log( MRsignal));
...
% FilterRho.value = 1 * 10^-3;%[m] HP-value for the radial filter
FilterRho.method = 'butter';
...
% assignin('base', 'hipass', shearWaveField(:,:,1,:,1,1));
shearWaveField_w = directionalWaveletFilter( squeeze(shearWaveField) );
assignin('base', 'dir_filt', shearWaveField_w(:,:,1,:,1,1));

% main inversion-----------------------------------------------------------
weigthingFactor = 4;% weigthing factor for the weigthed mean of shear wave speed and penetration rate (should be >= 1 or 0)
[c, Lambda, amplitude] = gradientInversion( shearWaveField_w, inplaneResolution, frequency, weigthingFactor);
