...
for prestr = {'MNI'}
    ...
    for kid = [1] % 2 3] % 1 2 3] %[2 3] %[1 4] %:4 %30,3 %1:length(t_id)
        ...
        % TPM2MNI(PROJ_DIR,Asubject);        %% hTPM2MNI                    
        
        eqname = 'equal';
        class_analysemodico_06.GMWM_MNI_calc_sblock(PROJ_DIR,Asubject,id,eqname);
        class_analysemodico_06.GMWM_MNI_plot(PROJ_DIR,id,eqname);
%         eqname = 'unequal';
%         class_analysemodico_06.GMWM_MNI_calc_sblock(PROJ_DIR,Asubject,id,eqname);
%         class_analysemodico_06.GMWM_MNI_plot(PROJ_DIR,id,eqname);
        
%     figure; plot(mFD,squeeze(abs(res.dASH_A(1,2,:))-abs(res.dASH_A(1,1,:))),'*')
...
if strcmp(MODE_calc_rer,'yes')
    ...
    save(fullfile('/store01_analysis/realtime/',['resdata8_' id '_' prestr '_rer.mat']),'res1','mFD');
end
if strcmp(MODE_calc_psf,'yes')
    res2 = calc_res_psf(PROJ_DIR,Asubject,prestr);
    save(fullfile('/store01_analysis/realtime/',['resdata8_' id '_' prestr '_rer.mat']),'res2','mFD');
end
if strcmp(MODE_calc_entropy,'yes')
    res3 = calc_res_entropy(PROJ_DIR,Asubject,BWthres,prestr);
    save(fullfile('/store01_analysis/realtime/',['resdata8_' id '_' prestr '_entropy.mat']),'res3','mFD');
end
