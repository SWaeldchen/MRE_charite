...
for kslice = 1:size(B0_individuel,3)      
   B(kslice,ksub) = mean2(abs(squeeze(B0_individuel(:,:,kslice,ksub)).*M(:,:,kslice)));
end
end

res.tstat_A = res.tstat_A*(-1);
...
[h,p] = ttest(atanh(res.r)); % "Fisher's z-transform of r" atanh(r) -> z-transformed r-values
disp('p-Wert');
disp(p);
disp('mean/sem von r');
...
disp('alle Probanden');
[rges,pges]=corrcoef(B(slc,:),squeeze(res.tstat_A(slc,4,:)-res.tstat_A(slc,1,:)));
disp(rges(2,1));
disp(pges(2,1));
