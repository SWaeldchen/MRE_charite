function modico_import(Asubject,modico,SCAN_SUB,RAWN_SUB)
disp('#### modico_import ####');
%% import mre data:
...
disp('Ser: mprage');
disp(Asubject.mprage);
Sermprage = Asubject.mprage;
...
if (length(Ndynma) == 1) % Smarttrigger
    cd(SCAN_SUB);
    names = dir('*.ima');
    ...
    for kser=Ndynma
        inc=0;        
        for kf=1:sum(SER(:,1)==Ndynma) %Nf=number of freq.
            inc_ser=inc_ser+1;
            inc=inc+1;
    ...
    for kser=Ndynph
        inc=0;        
        for kf=1:sum(SER(:,1)==Ndynph) %Nf=number of freq.
            inc_ser=inc_ser+1;
            inc=inc+1;
            tmp=SER(:,1) == kser & SER(:,2) == inc;
            TMP2 = fullfile(SCAN_SUB,names(I(tmp)).name);
...
%     dRL_dyn_ma = ['^' num2str(Asubject.dynma,'% 04.f') '_.*'];
%     dRL_dyn_ph = ['^' num2str(Asubject.dynma+1,'% 04.f') '_.*'];
%     
%     fdRL_dyn_ma = spm_select('FPList',SCAN_SUB,dRL_dyn_ma);
%     fdRL_dyn_ph = spm_select('FPList',SCAN_SUB,dRL_dyn_ph);
    ...
    fdRL_dyn_ma = '';
    fdRL_dyn_ph = '';
    for kNdynma = 1:length(Ndynma)
        dRL_dyn_ma = ['^' num2str(Ndynma(kNdynma),'% 04.f') '_.*'];    
        dRL_dyn_ph = ['^' num2str(Ndynph(kNdynma),'% 04.f') '_.*'];    
          
        tmp_fdRL_dyn_ma = spm_select('FPList',SCAN_SUB,dRL_dyn_ma);
        tmp_fdRL_dyn_ph = spm_select('FPList',SCAN_SUB,dRL_dyn_ph);
        
        fdRL_dyn_ma = [fdRL_dyn_ma; tmp_fdRL_dyn_ma];
        fdRL_dyn_ph = [fdRL_dyn_ph; tmp_fdRL_dyn_ph];
    end
    
  
    
%     dRL_dyn_ma1 = ['^' num2str(Ndynma(1),'% 04.f') '_.*'];
%     dRL_dyn_ma2 = ['^' num2str(Ndynma(2),'% 04.f') '_.*'];
%     dRL_dyn_ma3 = ['^' num2str(Ndynma(3),'% 04.f') '_.*'];
%     
%     dRL_dyn_ph1 = ['^' num2str(Ndynph(1),'% 04.f') '_.*'];
%     dRL_dyn_ph2 = ['^' num2str(Ndynph(2),'% 04.f') '_.*'];
%     dRL_dyn_ph3 = ['^' num2str(Ndynph(3),'% 04.f') '_.*'];
%     
%     fdRL_dyn_ma1 = spm_select('FPList',SCAN_SUB,dRL_dyn_ma1);
%     fdRL_dyn_ph1 = spm_select('FPList',SCAN_SUB,dRL_dyn_ph1);
%     
%     fdRL_dyn_ma2 = spm_select('FPList',SCAN_SUB,dRL_dyn_ma2);
%     fdRL_dyn_ph2 = spm_select('FPList',SCAN_SUB,dRL_dyn_ph2);
%     
%     fdRL_dyn_ma3 = spm_select('FPList',SCAN_SUB,dRL_dyn_ma3);
%     fdRL_dyn_ph3 = spm_select('FPList',SCAN_SUB,dRL_dyn_ph3);
%     
%     [fdRL_dyn_ma1;fdRL_dyn_ma2; fdRL_dyn_ma3];
%     fdRL_dyn_ph = [fdRL_dyn_ph1; fdRL_dyn_ph2; fdRL_dyn_ph3];
end
