function norm_epiorig2mni(DATA_DIR,TPMdir,voxsize)
disp('#### norm_epiorig2mni ####');
cd(DATA_DIR);
...
normdata{14} = 'EPI_iy_orig.nii';
normdata{15} = 'MAGf_orig.nii';
normdata{16} = 'dmyfield.nii';

if ~exist(fullfile(DATA_DIR,'y_MAGf_orig.nii'),'file');
    cd(DATA_DIR);
    disp('...est_normalise...');
    clear matlabbatch
    spm('defaults','fmri');
...
for knormdata = 1:length(normdata)
    ...
    if ~exist(fullfile(DATA_DIR,['w' normdata{knormdata}]),'file');
        ...
        spm('defaults','fmri');
        spm_jobman('initcfg');
        matlabbatch{1}.spm.spatial.normalise.write.subj.def = {fullfile(DATA_DIR,'y_MAGf_orig.nii')};
