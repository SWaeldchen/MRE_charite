function [ABSG PHI C]=mdev2D_no_filter(C,freqs,comps,weight,pixel_spacing,filt,BW) %#ok<INUSL>
    ...
    for k_freq = 1:length(freqs)
        ...
        %      end
                ...
                %numer_G=numer_G + 1000*om(k_freq).^2.*abs(U2).*abs(DU2);
                %denom_G=denom_G + abs(DU2).*abs(DU2);
