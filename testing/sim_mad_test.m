...
function [results, mad_vec] = sim_mad_test;
    ...
    %J_levels = 3:4;
    %n_J = numel(J_levels);
    ogs_levels = 0.08:0.02:0.12;
    n_ogs = numel(ogs_levels);
    butter_cuts = .03:.02:.07;
    ...
    for n = 1:n_noise
      for c = 1:n_cuts
          ...
          mad_vec(c, 1) = mad_eb(vec(results_butter(:,:,n, c)));
          toc
      end
    end
    ...
    for n = 1:n_noise
      %for j = 1:n_J
        for o = 1:n_ogs
          ...
          mad_vec(c, 2) = mad_eb(vec(results_ogs(:,:,n,c)));
          toc
        end
      %end
    end
