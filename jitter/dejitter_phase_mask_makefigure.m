function [s_d, stats] = dejitter_phase_mask_makefigure(s, mask, NORM, TOTAL_SHIFTS)
    ...
    if ~islogical(mask)
        dusp('Method requires logical mask')
        return
    end
    ...
    for n = 1:num_vols
        [s_resh(:,:,:,n), stats.norms(:,n), stats.distances(:,n)] = dejitter_vol(s(:,:,:,n), mask, TOTAL_SHIFTS, NORM);
    end
