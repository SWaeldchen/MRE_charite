function [ mu ] = norm_invert_for_mu_2( u1, u2, sigma1, sigma2, M1, M2, muReg, lambda )
...
Op = [D_u1'*MM1*D_u1; D_u2'*MM2*D_u2; lambda*gradMat(length(u1)-1)'*gradMat(length(u1)-1)];
...
vec = -[sigma1*D_u1'*MM1*u1(2:end-1); sigma2*D_u2'*MM2*u2(2:end-1); zeros(length(u1)-1,1)];
