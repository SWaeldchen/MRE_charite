%% function mredge_laplacian_unwrap_fft(info, prefs);
%
...
% USAGE:
%
%   unwraps data in 2, 3, or 4 dimensions using FFT
...
function w_u = mredge_laplacian_unwrap_fft(w, dims)
%parse
...
function x = dctn(x, dims)
    ...
    if dims == 2
        ...
        display('MREdge ERROR: Only 2D Laplacian currently implemented');
    end
end


function x = idctn(x, dims)
    sz = size(x);
    if dims == 2
        if numel(sz) < 4
            d4 = 1;
