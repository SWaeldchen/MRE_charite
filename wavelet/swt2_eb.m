function varargout = swt2_eb(x,n,varargin)
...
% Set DWT_Mode to 'per'.
old_modeDWT = dwtmode_eb('status','nodisp');
modeDWT = 'per';
dwtmode_eb(modeDWT,'nodisp');
...
for k = 1:n
    ...
    lf    = length(lo);
    first = [lf+1,lf+1];
    x  = wextend_eb('2D',modeDWT,x,[lf/2,lf/2]);
