function lowhigh = stretchlim_eb(varargin)
...
if (ndims(img) > 3)
    disp(message('images:stretchlim:dimTooHigh'))
end
...
if nargin == 2
    ...
    switch numel(tol)
        ...
        case 2
            if (tol(1) >= tol(2))
                disp(message('images:stretchlim:invalidTolOrder'))
            end
        otherwise
            disp(message('images:stretchlim:invalidTolSize'))
    end
end

if ( any(tol < 0) || any(tol > 1) || any(isnan(tol)) )
    disp(message('images:stretchlim:tolOutOfRange'))
end
