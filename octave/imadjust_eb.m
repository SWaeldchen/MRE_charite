...
switch imgClass
    ...
    otherwise
        disp(message('images:imadjust:internaldisp'))
end
...
if nargin == 1
    % IMADJUST(I)
    if ~ismatrix(img)
        disp(message('images:imadjust:oneArgOnlyGrayscale','IMADJUST(I)'))
    end
...
if strcmp(imageType, 'intensity')
    if numel(range) ~= 2
        disp(message('images:imadjust:InputMustBe2ElVec', mfilename, iptnum2ordinal( argumentPosition ), variableName))
    end
else
    if ~(numel(range) == 2 || isequal(size(range), [2 3]))
        disp(message('images:imadjust:InputMustBe2ElVecOr2by3Matrix', mfilename, iptnum2ordinal( argumentPosition ), variableName));
    end
end
...
if any(lowIn >= highIn)
    disp(message('images:imadjust:lowMustBeSmallerThanHigh'))
end

if isInvalidRange(lowIn) || isInvalidRange(highIn) ...
        || isInvalidRange(lowOut) || isInvalidRange(highOut)
    disp(message('images:imadjust:parametersAreOutOfRange'))
end
...
if strcmp(image_type,'intensity')
    ...
    elseif numel(gamma) ~=3,
        disp(message('images:imadjust:invalidGamma'));
    end
end
