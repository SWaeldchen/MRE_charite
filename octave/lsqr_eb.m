function [x,flag,relres,iter,resvec,lsvec] = lsqr_eb(A,b,tol,maxit,M1,M2,x0,varargin)
%LSQR   LSQR Method.
...
[atype,afun,afcnstr] = iterchk_eb(A);
if strcmp(atype,'matrix')
    % Check matrix and right hand side vector inputs have appropriate sizes
    [m,n] = size(A);
    nKnown = true;
    if size(b,1) ~= m
        error('RSHsizeMatchCoeffMatrix');
    end
else
    m = size(b,1);
end
...
if ((nargin >= 5) && ~isempty(M1))
    existM1 = true;
    [m1type,m1fun,m1fcnstr] = iterchk_eb(M1);
...
if ((nargin >= 6) && ~isempty(M2))
    existM2 = true;
    [m2type,m2fun,m2fcnstr] = iterchk_eb(M2);
...
u = b;
if xInit
    u = u - iterapp_eb('mtimes',afun,atype,afcnstr,x,varargin{:},'notransp');
end
beta = norm(u);
...
s = 0;
phibar = beta;
v = iterapp_eb('mtimes',afun,atype,afcnstr,u,varargin{:},'transp');
if ~nKnown
...
if existM2
    v = iterapp_eb('mldivide',m2fun,m2type,m2fcnstr,v,varargin{:},'transp');
    if any(~isfinite(v))
        flag = 2;
        relres = normr/n2b;
        iter = 0;
...
if existM1
    v = iterapp_eb('mldivide',m1fun,m1type,m1fcnstr,v,varargin{:},'transp');
    if any(~isfinite(v))
        flag = 2;
        relres = normr/n2b;
        iter = 0;
...
for ii = 1 : maxit
    if existM1
        z = iterapp_eb('mldivide',m1fun,m1type,m1fcnstr,v,varargin{:},'notransp');
        if any(~isfinite(z))
            flag = 2;
            iter = ii-1;
    ...
    if existM2
        z = iterapp_eb('mldivide',m2fun,m2type,m2fcnstr,z,varargin{:},'notransp');
        if any(~isfinite(z))
            flag = 2;
            iter = ii-1;
            resvec = resvec(1:iter+1);
    ...
    u = iterapp_eb('mtimes',afun,atype,afcnstr,z,varargin{:},'notransp') - alpha * u;
    beta = norm(u);
    u = u / beta;
    ...
    normr = abs(s) * normr;
    resvec(ii+1) = normr;
    vt = iterapp_eb('mtimes',afun,atype,afcnstr,u,varargin{:},'transp');
    if existM2
        vt = iterapp_eb('mldivide',m2fun,m2type,m2fcnstr,vt,varargin{:},'transp');
        if any(~isfinite(vt))
            flag = 2;
            iter = ii;
            resvec = resvec(1:iter+1);
    ...
    if existM1
        vt = iterapp_eb('mldivide',m1fun,m1type,m1fcnstr,vt,varargin{:},'transp');
        if any(~isfinite(vt))
            flag = 2;
            iter = ii;
            resvec = resvec(1:iter+1);
