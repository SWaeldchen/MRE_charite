...
function [ data ] = denoise_web_eb_test( dataPath,measIndex )
...
%     magImg = mean(mean(mean(abs(data(:,:,:,2,1,1)),4),5),6);
    ...
    % ADDED BY EB
    data.phase = dct_unwrap(data.phase, 2);
    %
    phase_ft = fft(double(data.phase), [],  4);
    ft = squeeze(phase_ft(:,:,:,p.fft_bin,:,:));
    % ADDED BY EB
    ft = sliceOffsetCorrection(ft, a.mreCubes(1).mreInfo);
    %
    size(ft)
    
    nii = make_nii([real(ft(:,:,:,:,1,1));imag(ft(:,:,:,:,1,1))]);
%     nii.img = [real(ft(:,:,:,:,1,1)); imag(ft(:,:,:,:,1,1))];
    ...
    % get virchow original
    load('info_and_prefs.mat');
    info.path = '/media/ericbarnhill/backup/projects/2018-03-12-virchow/results_wideband/AN_004_JB/';
