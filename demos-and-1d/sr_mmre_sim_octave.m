...
spike_randn_cutoff = 2.3;
bandwidth_coeffs = {2, 3, 4, 5};
n_coeffs = numel(bandwidth_coeffs);
laplacian = [1; -2; 1];
figuretally = 0;
...
% Generate base waves
x = repmat({zeros(128,1)}, [n_coeffs 1]);
increment = cell(numel(bandwidth_coeffs), 1);
base_wavelength = 128/(base_k);
for n = 1:n_coeffs
    increment{n} = pi/256*2.^bandwidth_coeffs{n};
end
...
x_super_denom = [];
for n = 1:numel(x)
    x_orig_num = cat(2, x_orig_num, 2.^(2*bandwidth_coeffs{n}) * abs(x{n}));
    x_orig_denom = cat(2, x_orig_denom, abs(x_lap{n}));
    x_down_num = cat(2, x_down_num, 2.^(2*bandwidth_coeffs{n}) *abs(x_down{n}));
    x_down_denom = cat(2, x_down_denom, abs(x_down_lap{n}));
    x_super_num = cat(2, x_super_num, 2.^(2*bandwidth_coeffs{n}) * abs_naives_polar{n});
