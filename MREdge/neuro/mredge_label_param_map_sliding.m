%% function stats = mredge_label_param_map_sliding(info, param)
...
%%
function mredge_label_param_map_sliding(info_an, prefs, param)

    [PARAM_SUB, STATS_SUB] = set_dirs(info_an, prefs, param);
    ...
    for f = 1:numel(stable_frequencies)
		disp(['Label Param Map: ',num2str(stable_frequencies(f)), 'Hz']);
        tic
...
function [PARAM_SUB, STATS_SUB] = set_dirs(info_an, prefs, param)
...
function label_param_map(STATS_SUB, param, tpm_image_path, param_file_path, noise_thresh, f)
    ...
    label_stats_path = fullfile(STATS_SUB, ['label_stats_stable_', param, '.csv']);
    num_labels = numel(label_nums);
    WM = 'White Matter';
    wm_sum = 0;
    wm_tally = 0;
    wm_path = fullfile(STATS_SUB, ['wm_stats_stable_', param, '.csv']);
