...
function [stable_filenames, stable_frequencies] = mredge_amplitudes_sliding(info, prefs)
    ...
    [freqs_sorted] = sort(info.driving_frequencies, 'ascend');
    for n = 1:nf-2
        stable_group = [freqs_sorted(n), freqs_sorted(n+1), freqs_sorted(n+2)];
		create_stable_amplitude(stable_group, AMP_SUB);
...
function create_stable_amplitude(stable_group, AMP_SUB)
    NIF_EXT = getenv('NIFTI_EXTENSION');
