...
function [c, Lambda, amplitude, shearWaveField, waveField] = kMDEV_anisodiff( MRsignal, inplaneResolution, frequency)

% smooth the complex MREsignal---------------------------------------------
sigma = 2.75 * 10^-3;%[m] filter strength for gaussien smoothing
MRsignalSmoothed = smoothMRsignal_anisodiff( MRsignal, inplaneResolution, sigma);
assignin('base', 'smoothed_signal_anisodiff', MRsignalSmoothed);
% sigma = 2.75 * 10^-3;%[m] filter strength for gaussien smoothing
...
% EB ADD
 waveField = kmdev_lsq_hhd(waveField);
...
% FilterTheta.flag = 'off';% flag for the directional filter
 ...
 FilterTheta.sigma = (2*pi)/8;% filter stength for the directional filter
