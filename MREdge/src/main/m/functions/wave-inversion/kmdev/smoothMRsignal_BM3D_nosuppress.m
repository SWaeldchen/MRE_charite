...
function MRsignalSmoothed = smoothMRsignal_BM3D_nosuppress( MRsignal)
    ...
    for iComponent = 1 : nComponent
        for iTimestep = 1 : nTimestep
            for iSlice = 1 : nSlice
				...
				[~, sigSmooth_real_norm] = BM3D(1, norm_real, sigma, 'lc');
				[~, sigSmooth_imag_norm] = BM3D(1, norm_imag, sigma, 'lc');    
