...
bx_ = zeros(nx, ny, nz, nf*3);
Uxxy_ = zeros(nx, ny, nz, nf*3);
    ...
    for j=2:ny-1
        for k=2:nz-1
            for f = 1:nf
                ...
                % direct vectors
                ind = (f-1)*3 +1;
                bx_(i,j,k,ind:ind+2) = bx;
                Uxxy_(i,j,k,ind:ind+2) = Uxxy;

                b(i,j,k,ind:ind+2) = bx + by + bz;

                p(i,j,k,ind:ind+2) = [Ux(i,j,k,f)*dx*dy*dz; Uy(i,j,k,f)*dx*dy*dz; Uz(i,j,k,f)*dx*dy*dz];
            ...
            G(i,j,k) = (-1/dot(vec(b(i,j,k,:)),vec(b(i,j,k,:)))) * dot(vec(b(i,j,k,:)),rho*om_sq.*vec(p(i,j,k,:)));
        end
    end
end
