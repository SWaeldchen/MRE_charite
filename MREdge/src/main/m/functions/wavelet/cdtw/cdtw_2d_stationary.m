function w = cdtw_2d_stationary(x, J, Faf, af)

% Stationary Version of Dual-Tree Complex 2D Discrete Wavelet Transform
%
% USAGE:
...
for m = 1:2
    for n = 1:2
        %[lo, w{1}{m}{n}] = afb2D_stationary(x, Faf{m}, Faf{n});
        [lo, w{1}{m}{n}] = afb2D_stationary(x, af{m}, af{n});
        for j = 2:J
            af_atrous = atrous(af, j);
            [lo, w{j}{m}{n}] = afb2D_stationary(lo, af_atrous{m}, af_atrous{n});
        end
        w{J+1}{m}{n} = lo;
        size(lo)
    end
