function y = icdtw_2d_stationary(w, J, Fsf, sf)
...
for m = 1:2
    for n = 1:2
        lo = w{J+1}{m}{n};
        for j = J:-1:2
			      sf_atrous = atrous(sf, j);
            lo = sfb2D_stationary(lo, w{j}{m}{n}, sf_atrous{m}, sf_atrous{n});
        end
        lo = sfb2D_stationary(lo, w{1}{m}{n}, Fsf{m}, Fsf{n});
        y = y + lo;
    end
end
