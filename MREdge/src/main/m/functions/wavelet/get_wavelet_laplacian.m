...
function U_laplacian = get_wavelet_laplacian(U, spacing)

if ndims(U) == 5
    U_laplacian = get_wavelet_laplacian_5d(U, spacing);
elseif ndims(U) == 4
    U_laplacian = get_wavelet_laplacian_4d(U, spacing);
elseif ndims(U) == 3
    U_laplacian = get_wavelet_laplacian_3d(U, spacing);
else
...
function U_laplacian = get_wavelet_laplacian_5d(U, spacing, twoD) 
        U_laplacian = zeros(size(U));
        for n = 1:size(U, 5)
            temp = get_wavelet_laplacian_4d(U(:,:,:,:,n), spacing, twoD);
            U_laplacian(:,:,:,:,n) = temp;
        end
end


function U_laplacian = get_wavelet_laplacian_4d(U, spacing, twoD)
    U_laplacian = zeros(size(U));
    if (nargin < 3)
        twoD = 0;
    end
...
function U_laplacian = get_wavelet_laplacian_3d(U, spacing, twoD)
    if (nargin < 3)
        twoD = 0;
    end
    if (twoD == 0)
