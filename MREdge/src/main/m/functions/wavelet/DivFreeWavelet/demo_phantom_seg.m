%% Denoise Flow Phantom Data with incorrect segmentation
%
% The following compares divergence-free wavelet denoising with existing
% methods on a flow phantom with incorrect segmentation.
%
% see demo_phantom.m for performances with correct segmentation
...
% Crop to pipe for faster runtime
...
% Set parameters
imThresh = 2000;                % Image threshold for segmentation
imMask = (imMag>imThresh)*1;    % Segmentation mask
imCorrThresh = 7000;            % Correct image threshold
imCorrMask = (imMag>imCorrThresh)*1;    % Correct segmentation mask
...
% Mask flow data with incorrect segmentation
...
[vNRMSE_Noise,vMagErr_Noise,angErr_Noise] = calcVelError(imCorrMask,vx,vy,vz,vxN,vyN,vzN);
...
[vNRMSE_DFWsm,vMagErr_DFWsm,angErr_DFWsm] = calcVelError(imCorrMask,vx,vy,vz,vxDFWsm,vyDFWsm,vzDFWsm);
...
[vNRMSE_DFWsms,vMagErr_DFWsms,angErr_DFWsms] = calcVelError(imCorrMask,vx,vy,vz,vxDFWsms,vyDFWsms,vzDFWsms);
...
[vNRMSE_FDM,vMagErr_FDM,angErr_FDM] = calcVelError(imCorrMask,vx,vy,vz,vxFDM,vyFDM,vzFDM);
...
[vNRMSE_RBF,vMagErr_RBF,angErr_RBF] = calcVelError(imCorrMask,vx,vy,vz,vxRBF,vyRBF,vzRBF);
