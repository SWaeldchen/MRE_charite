...
void print_array(char* prefix,int* array,int length)
{
	int i;
	printf("\t%s:\t",prefix);
	for (i=0; i<length; i++)
...
void print_filter(char* prefix,scalar_t* array,int length)
{
	int i;
	printf("\t%s:\t",prefix);
	for (i=0; i<length; i++)
...
void print_plan(struct dfwavelet_plan_s* plan)
{
	printf("Parameters for Input/Output:\n");
	print_array("use_gpu",&(plan->use_gpu),1);
	print_array("numdims",&(plan->numdims),1);
	print_array("imSize ",plan->imSize,plan->numdims);
	printf("\tres     :\t%.1f, %.1f, %.1f\n",plan->res[0],plan->res[1],plan->res[2]);
	print_array("numPixel",&(plan->numPixel),1);
	print_array("numCoeff",&(plan->numCoeff),1);
	printf("\tmemory (float):\t%.1f MegaBytes\n",plan->numCoeff*3.*4./1024./1024.);

	printf("Parameters for each wavelet transform:\n");
	print_array("minSize",plan->minSize,plan->numdims);
	print_array("numCoarse",&(plan->numCoarse),1);
	print_array("waveSizes",plan->waveSizes,plan->numdims*(plan->numLevels+2));
	print_array("numLevels",&(plan->numLevels),1);
	print_array("randShift",plan->randShift,plan->numdims);

	printf("Parameters for filters:\n");
	print_array("filter_Len",&(plan->filterLen),1);
	print_filter("lod0\t",plan->lod0,plan->filterLen);
	print_filter("hid0\t",plan->hid0,plan->filterLen);
	print_filter("lor0\t",plan->lor0,plan->filterLen);
	print_filter("hid0\t",plan->hid0,plan->filterLen);

	print_filter("lod1\t",plan->lod1,plan->filterLen);
	print_filter("hid1\t",plan->hid1,plan->filterLen);
	print_filter("lor1\t",plan->lor1,plan->filterLen);
	print_filter("hid1\t",plan->hid1,plan->filterLen);
}
