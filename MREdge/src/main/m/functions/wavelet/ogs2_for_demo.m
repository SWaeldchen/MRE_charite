function [x, cost] = ogs2_for_demo(y, K1, K2, lam, pen, rho, Nit, level, tree, dir)
...
% Perform some error checking
	...
	for it = 1 : Nit
		...
		x = y./v;
		plotFunctions(y, r, v, x, it, level, tree, dir);
	end
end

function plotFunctions(y, r, v, x, it, level, tree, dir) 
	szy = size(y);
	figure(1);  colormap('default');
	subplot(3, 2, 2);
	surf(abs(y)); title(['Level ', num2str(level), ' Tree ', num2str(tree), ' Dir ', num2str(dir), ' Iter ', num2str(it)]); 
	axis([0 szy(1) 0 szy(2) 0 5*level]); freezeColors;
	subplot(3, 2, 3);
	surf(r); title(['Bounding Function']); axis([0 szy(1) 0 szy(2) 0 3]); freezeColors;
	subplot(3, 2, 4);
	surf(v); title(['Weighting Function']); axis([0 szy(1) 0 szy(2) 0 3]); freezeColors;
	subplot(3, 2, 5);
	surf(abs(x)); title(['Thresholded Coefficients']); axis([0 szy(1) 0 szy(2) 0 5*level]); freezeColors;
	pause(0.01);
end
