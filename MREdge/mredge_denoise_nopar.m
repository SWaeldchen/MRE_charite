%% function mredge_denoise_nopar(info, prefs);
%
...
function mredge_denoise_nopar(info, prefs)
    ...
    for d = 1:numel(FT_DIRS);
        for f_num = 1:numel(info.driving_frequencies)
            ...
            for c = 1:3
                display(num2str(c));
                wavefield_path = fullfile(FT_DIRS{d}, num2str(f), num2str(c), mredge_filename(f, c, NIF_EXT)); %#ok<PFBNS>
                ...
                elseif strcmp(prefs.denoise_strategy, 'z_xy_ampguided') == 1 %#ok<PFBNS>
					amp = abs(wavefield_img);
                    wavefield_img = dtdenoise_z_auto_noise_est(wavefield_img, prefs.denoise_settings.z_level, prefs.denoise_settings.z_thresh_factor);
                    wavefield_img = dtdenoise_xy_pca_mad_amp(wavefield_img, amp, prefs.denoise_settings.xy_thresh_factor, prefs.denoise_settings.xy_num_spins);
