%% function mredge_organize_acquisition_octave(info)
...
% guided by acquisition info object. May contain tweaks for octave compatibility
%
% INPUTS:
%
...
function mredge_organize_acquisition_octave(info, prefs)
...
if ~isempty(info.phase)
	...
	for n = info.phase
	   mredge_dicom_to_nii_octave(n, data_table, PHASE_SUB, is_time_series, info);
	end
...
if ~isempty(info.magnitude)
    ...
    for n = info.magnitude
      mredge_dicom_to_nii_octave(n, data_table, MAG_SUB, is_time_series, info);
    end
    if info.all_freqs_one_series == 1
