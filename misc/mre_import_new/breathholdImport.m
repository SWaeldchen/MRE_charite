function [mreCubes otherMagnitudeCubes otherComplexCubes]=breathholdImport(inputPath,outputPath)
...
%         selector(index)=mreInfo.nTimesSteps>=8 && mreInfo.nDirs>=3;
        selector(index)=mreInfo.nTimesSteps>=8;
    end
end
% selector(1)=false;
...
dirDimension=5;
counter=1;
for index=1:3:numel(mreCubes)
    indices=index:(index+2);
    concatMreCubes(counter)=concatComplexCubes( mreCubes(indices),dirDimension);
    counter=counter+1;
end
freqDimension=6;
%% concat freqs
mreCubes=concatComplexCubes( concatMreCubes,freqDimension);
