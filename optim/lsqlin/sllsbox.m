...
if nargin < 4
  disp('optim:sllsbox:NotEnoughInputs')
end
if nargin < 5
    xstart = []; 
end
...
if ischar(kmax)
   if isequal(lower(kmax),'max(1,floor(numberofvariables/2))')
      ...
      disp('optim:sllsbox:InvalidMaxPCGIter')
   end
end
if ischar(typx)
   if isequal(lower(typx),'ones(numberofvariables,1)')
      ...
      disp('optim:sllsbox:InvalidTypicalX')
   end
end
checkoptionsize('TypicalX', size(typx), numberOfVariables);

if n == 0
   disp('optim:sllsbox:InvalidN')
end
...
lb(arg2) = -inf;
if any(ub == lb) 
   disp('optim:sllsbox:EqualLowerUpperBnd')
elseif min(ub-lb) <= 0
   disp('optim:sllsbox:InconsistentBnds')
end
lvec = lb; uvec = ub;
