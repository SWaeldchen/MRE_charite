...
% Check lb length
if lenlb > nvars
    disp('optimlib:checkbounds:IgnoringExtraLbs');
    lb = lb(1:nvars);
    lenlb = nvars;
elseif lenlb < nvars % includes empty lb case
    if lenlb > 0
        % lb is non-empty and length(lb) < nvars.
        disp('optimlib:checkbounds:PadLbWithMinusInf');
    end
...
% Check ub length
if lenub > nvars
    disp('optimlib:checkbounds:IgnoringExtraUbs');
    ub = ub(1:nvars);
    lenub = nvars;
elseif lenub < nvars % includes empty ub case
    if lenub > 0
        % ub is non-empty and length(ub) < nvars.
        disp('optimlib:checkbounds:PadUbWithInf');
    end
...
if any(eq(ub, -inf))
    disp('optimlib:checkbounds:MinusInfUb');
elseif any(eq(lb,inf))
    disp('optimlib:checkbounds:PlusInfLb');
end
