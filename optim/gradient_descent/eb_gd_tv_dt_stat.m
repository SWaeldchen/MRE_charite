function x_rec = eb_gd_tv_dt_stat(x, niter, lambda, tau, epsilon)
...
[Faf, Fsf] = FSfarras;
[af, sf] = dualfilt1;
x_dwt = cdtw_2d_stationary(x, J, Faf, af);
e = zeros(niter, 1);
fid = zeros(niter, 1);
reg = zeros(niter, 1);
for n = 1:niter
    ...
    fid(n) = fid_;
    reg(n) = reg_;
    x_rec_dwt = cdtw_2d_stationary(x_rec, J, Faf, af);
    for j = 1:J
        % loop thru subbands
        for s1 = 1:2
            for s2 = 1:3
    ...
    x_rec = icdtw_2d_stationary(x_rec_dwt, J, Fsf, sf);
end
