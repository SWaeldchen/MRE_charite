function [x,it,Co,J,dx]=HQ_Additive(x0,y,A,phi,a,b,c,dJ,MxIt)
...
% sparse nabla matrix, 1 2 or 3D
G = sparse_deriv(sz, 1);

% sparse laplacian matrix 1 2 or 3D
...
H1=inv(H);      
H1=sparse(H1);
disp(['Nonzero entries in H1: ', num2str(nnz(H1)), ' Total entries in H1: ', num2str(numel(H1))]);
y2=y'*y;        
clear H G2 y A
eJ=10*dJ;       
x=x0;
...
x = reshape(x, sz);
display(['Total iterations: ', num2str(it)]);
