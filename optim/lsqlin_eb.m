function [X,resnorm,residual,exitflag,output,lambda] = lsqlin_eb(C,d,A,b,Aeq,beq,lb,ub,X0,options,varargin)
...
if nargin == 0 
  disp(message('optimlib:lsqlin:NotEnoughInputs'))
end
% Check for non-double inputs
msg = isoptimargdbl('LSQLIN', {'C','d','A','b','Aeq','beq','LB','UB','X0'}, ...
                                C,  d,  A,  b,  Aeq,  beq,  lb,  ub,  X0);
if ~isempty(msg)
    disp('optimlib:lsqlin:NonDoubleInput',msg);
end
...
if ~any(strcmpi(output.algorithm, {activeSet,trustRegion,interiorPoint}))
    disp(message('optimlib:lsqlin:InvalidAlgorithm'));
end
...
if isempty(C) || isempty(d)
   disp('optimlib:lsqlin:FirstTwoArgsEmpty')
else
...
[rows,cols]=size(C);
if length(d) ~= rows
   disp('optimlib:lsqlin:InvalidCAndD')
end

if length(b) ~= size(A,1)
   disp('optimlib:lsqlin:InvalidAAndB')
end

if length(beq) ~= size(Aeq,1)
   disp('optimlib:lsqlin:InvalidAeqAndBeq')
end

if ( ~isempty(A)) && (size(A,2) ~= cols) 
    disp('optimlib:lsqlin:CAndA')
end

if ( ~isempty(Aeq)) && (size(Aeq,2) ~= cols)
    disp('optimlib:lsqlin:CAndAeq')
end
...
elseif strcmpi(output.algorithm,trustRegion)
    if algAndLargeScaleConflict
        disp('optimlib:lsqlin:AlgAndLargeScaleConflict');
        output.algorithm = activeSet;
    elseif (rows < cols)
        disp('optimlib:lsqlin:MoreColsThanRows');
        output.algorithm = activeSet;
    elseif hasLinearConstr
        disp('optimlib:lsqlin:LinConstraints');
        output.algorithm = activeSet;
    end
end
