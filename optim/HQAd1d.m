...
if nargin <7    c=0;                end
if nargin <8    dJ=1e-5*norm(y)/length(y);  end
if nargin <9    MxIt=Inf;           end
if nargout >3   JJ=[];              end
if nargout >4   dx=[];              end
if c==0 if phi=='log'       c=1/(a^2);
        elseif phi=='sqr'   c=1/sqrt(a);
        elseif phi=='hub'   c=1;
end;    end
n=length(x0);   x0=x0(:);           y=y(:);  
if A==1 A2=eye(n,n);    z=2*y;
else    A2=A'*A;        z=A'*y;     z=2*z;
end
G=eye(n,n)-diag(ones(n-1,1),1);     G(n,:)=[];
G2=2*eye(n,n)-diag(ones(n-1,1),1);
G2=G2-diag(ones(n-1,1),-1);         G2(1)=1;G2(n,n)=1;
H=2*A2+b*c*G2;
Co=cond(H);
H1=inv(H);      H1=sparse(H1);
y2=y'*y;        clear H G2 y A
