function [mag, phi, snr] = ESP_v0_51b(U, freqvec, spacing, super_factor, hipass_strat)
  ...
  % denoise
    ...
    parfor m = 1:d5
			display('Denoising z-3D')
     		U(:,:,:,:,m) = DT_3D_v0_51b(U(:,:,:,:,m));
    end
    
    assignin('base', 'U_denoise', U);

    if hipass_strat == -1
        U = hipass_butter_3d_nopad(8, 0.8, U);
    else
        U = hodge_decomp(U, hipass_strat);
    end
